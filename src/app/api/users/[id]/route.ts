...
async function syncCompanyStatus(
  ...
  try {
    const companyModel =
      (prisma as any).company || (prisma as any).Company;
...
export async function PUT(
  ...
  try {
    ...
    if (role) {
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Insufficient permissions to assign SUPER_ADMIN role",
          },
          { status: 403 }
        );
      }
    ...
    if (
      payload.role === "ADMIN" &&
      currentUser.createdBy !== payload.userId
    ) {
      return NextResponse.json(
        { error: "Unauthorized access to update this user" },
        { status: 403 }
      );
    }
    ...
    if (payload.role === "ADMIN") {
      ...
      if (
        ...
        requestingAdmin.status === "INACTIVE"
      ) {
        return NextResponse.json(
          {
            error:
              "Your Admin account is inactive. Super Admin must activate your admin access first.",
          },
          { status: 403 }
        );
      }
    ...
    let resolvedCompanyName: string | null =
    ...
    const targetStatus: UserStatus = status
    ...
    // Owner Protections
    const isOwner =
      currentUser.companyRef?.ownerUserId === id;
    ...
    if (isOwner) {
      if (updateRole === "USER") {
        return NextResponse.json(
          {
            error:
              "The owner of the organization must be an ADMIN and cannot be demoted.",
          },
          { status: 400 }
        );
      }
      ...
      if (!targetIsActive) {
        return NextResponse.json(
          {
            error:
              "The owner of the organization cannot be deactivated.",
          },
          { status: 400 }
        );
      }
    ...
    if (targetIsActive) {
      if (currentUser.createdBy) {
        const creatorAdmin = await prisma.user.findUnique({
          where: { id: currentUser.createdBy },
        });

        if (
          creatorAdmin &&
          (!creatorAdmin.isActive ||
            creatorAdmin.status === "INACTIVE")
        ) {
          return NextResponse.json(
            {
              error:
                "Parent Admin account is inactive. Super Admin must activate the admin access first.",
            },
            { status: 400 }
          );
        }
      ...
      if (resolvedCompanyId) {
        ...
        if (
          comp &&
          (!comp.isActive || comp.status === "INACTIVE")
        ) {
          return NextResponse.json(
            {
              error:
    ...
    await syncCompanyStatus(
      resolvedCompanyId,
      resolvedCompanyName
    );

    if (
      currentUser.companyId !== resolvedCompanyId ||
      currentUser.company !== resolvedCompanyName
    ) {
      await syncCompanyStatus(
...
export async function DELETE(
  ...
  try {
    ...
    if (
      targetUser.companyRef?.ownerUserId === id
    ) {
      return NextResponse.json(
        {
          error:
            "The owner of the organization cannot be deleted. Transfer ownership first.",
        },
        { status: 400 }
      );
    }

    // An Admin who still has Users under them cannot be deleted.
...
export async function PATCH(
  ...
  try {
      ...
      if (payload.role === "ADMIN") {
        ...
        if (
          ...
          return NextResponse.json(
            {
              error:
                "Unauthorized access to this user",
            },
            { status: 403 }
          );
        }
    ...
    if (action === "reset-password") {
      ...
      // Instead, dispatch a secure PASSWORD_RESET link.
      ...
      if (!mailResult.success) {
        return NextResponse.json(
          {
            error: `Failed to send password reset link to ${targetUser.email}`,
          },
          { status: 500 }
        );
      }
      ...
      return NextResponse.json({
        message: `A password reset link has been sent to ${targetUser.email}.`,
      });
    }
    ...
    if (action === "toggle-status") {
      ...
      if (
        user.companyRef?.ownerUserId === id &&
        !nextIsActive
      ) {
        return NextResponse.json(
          {
      ...
      if (nextIsActive && user.createdBy) {
        ...
        if (
          ...
          return NextResponse.json(
            {
              error:
                "Parent Admin account is inactive. Super Admin must activate the admin first.",
            },
            { status: 400 }
          );
        }
      ...
      await logAuditEvent({
        ...
        category: isTargetAdmin
          ? "Admin Management"
          : "User Management",
        severity: nextIsActive
          ? "SUCCESS"
          : "WARNING",
        actorName: payload.email.split("@")[0],
        actorEmail: payload.email,
